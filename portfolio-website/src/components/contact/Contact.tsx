...
const Contact = () => {
    useEffect(() => {
        const emailWindow = window.open('mailto:jadattilobusiness@gmail.com');
        // Check if the email client was opened
        const interval = setInterval(() => {
            if (emailWindow && emailWindow.closed) {
                clearInterval(interval);
                // If email client was closed (canceled), go back a page
                window.history.back();
            }
        }, 1000);
